...
const theme = tv({
	slots: {
		base: 'focus:ring-none inline-flex w-full items-center justify-between border shadow-sm focus:outline-none',
		triggerIcon: ' ',
	...
	variants: {
		variant: {
			nude: {
				// base: 'border-gray-300 bg-white hover:bg-gray-50 text-gray-700',
				// content: 'border shadow-lg mt-1 w-full rounded-md',
				// triggerIcon: 'text-gray-500',
				// label: 'text-gray-400',
				// scrollButton: 'text-gray-400',
				// scrollIcon: 'text-blue-500',
...
export function Select<T = string>(props: SelectProps<T>) {
	...
	const OpenIcon = useMemo(() => {
		const name = isOpen ? icons.chevronUp : icons.chevronDown;
		return <Icon icon={name} className="size-4 text-blue-500" />;
	}, [isOpen, icons]);
...
export function SelectItem({ value, children }: SelectItemProps) {
	...
	return (
		...
		>
			...
			{/*className="absolute right-3 data-[state=checked]:block data-[state=unchecked]:hidden"*/}
